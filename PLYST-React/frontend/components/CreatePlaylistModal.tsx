...
export default function CreatePlaylistModal({
  ...
  const handleSearchTracks = async () => {
    ...
    setIsSearching(true);
    try {
      // 검색어를 제목과 아티스트로 분리 시도
      const parts = searchQuery.split(" - ");
      const trackTitle = parts[0] || searchQuery;
      const artist = parts[1] || "";
      
      const info = await getTrackInfo(trackTitle, artist);
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg max-h-[85vh] overflow-hidden backdrop-blur-2xl bg-black/40 border border-white/20 rounded-3xl shadow-2xl"
          >
            {/* Header */}
